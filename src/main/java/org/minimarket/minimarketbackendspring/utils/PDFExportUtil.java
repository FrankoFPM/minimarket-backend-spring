...
import org.minimarket.minimarketbackendspring.dtos.*;
import org.minimarket.minimarketbackendspring.services.impl.DescuentoPromocionServiceImpl;
...
public class PDFExportUtil {
    ...
    @Autowired
    private DescuentoPromocionServiceImpl descuentoPromocionService;
    ...
    public byte[] exportarBoletaFromPedidoPDF(List<DetallePedidoDTO> items, String idUsuario, String numeroTransaccion) throws IOException {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        PdfWriter writer = new PdfWriter(out);
        PdfDocument pdf = new PdfDocument(writer);
        Document document = new Document(pdf);
        ...
        agregarHeaderBoleta(document, colorPrimario, colorFondo);
        agregarInfoBoletaBonita(document, numeroTransaccion, idUsuario, colorPrimario, colorFondo);
        agregarTablaPedidoBonita(document, items, colorPrimario, colorSecundario, colorDescuento, colorFondo);
        agregarResumenPedidoTotalesBonito(document, items, colorPrimario, colorSecundario, colorDescuento, colorFondo);
        agregarPieBoletaBonito(document, colorTexto);
    ...
    private void agregarTablaPedidoBonita(Document document, List<DetallePedidoDTO> items,
                                           DeviceRgb colorPrimario, DeviceRgb colorSecundario,
                                           DeviceRgb colorDescuento, DeviceRgb colorFondo) {
        ...
        for (DetallePedidoDTO item : items) {
            ...
            // Precio Original
            BigDecimal precioOrig = item.getPrecioUnitario();
            table.addCell(crearCeldaBonita("S/ " + String.format("%.2f", precioOrig), colorFilaFondo, false));
            ...
            DescuentoPromocionDTO descuentoPromocion = descuentoPromocionService.findByProducto(item.getIdProducto()).stream().findFirst().orElse(null);

            if (descuentoPromocion != null && descuentoPromocion.getPorcentaje() != null && descuentoPromocion.getPorcentaje().compareTo(BigDecimal.ZERO) > 0) {
                BigDecimal montoDescuento = precioOrig.multiply(descuentoPromocion.getPorcentaje().divide(BigDecimal.valueOf(100)));
                Paragraph descP1 = new Paragraph("💸 -S/ " + String.format("%.2f", montoDescuento))
                        .setFontSize(9)
                        .setBold()
                        .setFontColor(colorDescuento)
                        .setMarginBottom(2);

                Paragraph descP2 = new Paragraph("(" + String.format("%.1f", descuentoPromocion.getPorcentaje()) + "% OFF)")
                        .setFontSize(8)
                        .setItalic()
                        .setFontColor(colorDescuento);
    ...
    private void agregarResumenPedidoTotalesBonito(Document document, List<DetallePedidoDTO> items,
                                             DeviceRgb colorPrimario, DeviceRgb colorSecundario,
                                             DeviceRgb colorDescuento, DeviceRgb colorFondo) {
        ...
        for (DetallePedidoDTO item : items) {
            DescuentoPromocionDTO descuentoPromocion = descuentoPromocionService.findByProducto(item.getIdProducto()).stream().findFirst().orElse(null);
            BigDecimal precioOrig = item.getPrecioUnitario();
            BigDecimal precioFinal = descuentoPromocion != null && descuentoPromocion.getPorcentaje() != null && descuentoPromocion.getPorcentaje().compareTo(BigDecimal.ZERO) > 0
                    ? precioOrig.multiply(BigDecimal.ONE.subtract(descuentoPromocion.getPorcentaje().divide(BigDecimal.valueOf(100))))
                    : precioOrig;
            BigDecimal cantidad = BigDecimal.valueOf(item.getCantidad());
